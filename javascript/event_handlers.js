...
function registerClickEvent(refreshButton, invisible_button_id_selector) {
...
onUiUpdate(function() {
  ...
  registerClickEvent(gradioApp().querySelector('#refresh_sd_model_checkpoint'), '#cp_modelpreview_xd_refresh_sd_model');
  ...
  gradioApp().querySelectorAll('#img2img_script_container button, #txt2img_script_container button').forEach(function(button) {
    if(button.innerHTML === 'Refresh models') {
      registerClickEvent(button, '#lo_modelpreview_xd_refresh_sd_model');
    }
  });

  // Sync the refresh hypernetworks list button
  registerClickEvent(gradioApp().querySelector('#refresh_sd_hypernetwork'), '#hn_modelpreview_xd_refresh_sd_model');
})
